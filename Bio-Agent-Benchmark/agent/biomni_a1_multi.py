"""
Biomni A1 Multi-Agent adapter for Bio-Agent-Benchmark.

Extends BiomniA1Agent with:
  - Rule-based task classification (from prompt keywords)
  - Curated tool injection per task (bypasses ToolRetriever)
  - Task-specific structured prompts (persona + mandatory steps)
  - Output post-processing (format normalization)

Comparison baseline: agent/biomni_a1_baseline.py (original, 37% accuracy)
...
class BiomniA1MultiAgent:
    ...
    def predict(self, prompt, task_id="unknown"):
        ...
        # Step 1.5: Check if this task has a specialist pipeline (Exp#4)
        ...
        if pipeline:
            ...
            try:
                a1 = self._pool.get()
                try:
                    return self._run_specialist(a1, task_name, pipeline, prompt, task_id)
                finally:
                    self._pool.put(a1)
